...
try:
    ...
    class ConvexHull:
        """Pure Python 2D Monotone Chain Convex Hull Fallback."""
        def __init__(self, points: np.ndarray):
            ...
            self.vertices = np.array([np.where((points == hp).all(axis=1))[0][0] for hp in hull_pts])
            # Compute 2D polygon area
            hull_arr = np.array(hull_pts)
            x, y = hull_arr[:, 0], hull_arr[:, 1]
            self.volume = 0.5 * float(np.abs(np.dot(x, np.roll(y, 1)) - np.dot(y, np.roll(x, 1))))
